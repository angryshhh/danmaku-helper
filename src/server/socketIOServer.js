  ...
  reactSocket.on('roomId', reactSocketData => {
    ...
    tcpSocket.on('data', tcpSocketData => {
      ...
      if((tempBuffer.readInt32LE(0) + 4) === tempBuffer.length) {
        // tempBuffer is a whole message, now handle it
        const obj = parseTCPMessage(tempBuffer);
        switch(obj.type) {
          case 'loginres':
        ...
        tempBuffer = Buffer.alloc(0); // zero it when it is handled
      } else {
        // tempBuffer is not completed, waiting next data
      }
    });
